...
yFile = myArgs[[1]];
maxTotalPoints = Quiet[Check[ToExpression[Last[myArgs]], $Failed]];
nPts = If[Length[myArgs] >= 3,
  Quiet[Check[ToExpression[myArgs[[2]]], $Failed]],
  10
];
minWidth = If[Length[myArgs] >= 4,
  Quiet[Check[ToExpression[myArgs[[3]]], $Failed]],
  1/1000000
];
outputFile = "x.txt";
...
continuumVector[x_?NumericQ, j_Integer] := Module[{zv, pref},
  ...
  N[
    Join[
      {pref*(2/zv), 0},
      Table[pref*Nlist[n, zv, j], {n, 0, nulllist[[1]]}]
    ],
    prec
  ]
];
...
worstFunctional[x_?NumericQ] := Module[
  ...
  values = functionalValue[x, #] & /@ jValues;
...
Print["=== check_sampling.m ==="];
Print["  yFile             = ", yFile];
Print["  nPts              = ", nPts];
Print["  minWidth          = ", minWidth];
Print["  maxTotalPoints    = ", maxTotalPoints];
Print["  current points    = ", Length[currentSamplePoints]];
Print["  sampled J entries = ", Length[jValues]];
...
(* Keep enough printed precision for repeated copy/paste cycles.  The outer
   SetPrecision wrapper matches test17.m's xSamples assignment style. *)
literal = "xSamples = SetPrecision[" <>
  ToString[InputForm[SetPrecision[mergedPoints, 80]]] <>
  ", prec];";
